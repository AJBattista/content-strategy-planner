...
export const FIT_LABEL_CLASSES: Record<BenchmarkFitLabel, { text: string; bg: string; dot: string }> = {
  'Strong Fit':   { text: 'text-status-strong',  bg: 'bg-status-strong/10',  dot: 'bg-status-strong' },
  'Viable':       { text: 'text-status-neutral',  bg: 'bg-status-neutral/10',  dot: 'bg-status-neutral' },
  'Weak Fit':     { text: 'text-status-weak',     bg: 'bg-status-weak/10',     dot: 'bg-status-weak' },
  'Overextended': { text: 'text-status-caution',  bg: 'bg-status-caution/10',  dot: 'bg-status-caution' },
};
